...
.. glossary::
   ...
   ``-mce``
     Related to `Windows XP Media Center Edition <https://en.wikipedia.org/wiki/Windows_XP_Media_Center_Edition>`_. When the game is launched with this command-line option, the ``%SystemRoot%\ehome\ehshell.exe`` executable will be run upon exiting the game, or if Windows Virtual-Memory is not enabled, if the executable is present.
